/*
Template Name: IndiaIVF - Bootstrap 5 Admin & Dashboard Template
Author: Themesdesign
Version: 1.2.0.
...
(function($) {
    ...
    var language = localStorage.getItem('IndiaIVF-language');
    // Default Language
    var default_lang = 'en';
    ...
    function setLanguage(lang) {
        if (document.getElementById("header-lang-img")) {
            ...
            localStorage.setItem('IndiaIVF-language', lang);
            language = localStorage.getItem('IndiaIVF-language');
            getLanguage();
        }
    }
    ...
    function initLeftMenuCollapse() {
        ...
        $(window).on('load', function() {
            ...
            // Restore sidebar size from localStorage if available
            var savedSidebarSize = localStorage.getItem('sidebar-size');
            if (savedSidebarSize) {
                document.body.setAttribute('data-sidebar-size', savedSidebarSize);
            } else if (window.innerWidth >= 1024 && window.innerWidth <= 1366) {
                // If no saved state and screen is medium size, set to 'sm'
        ...
        $('#vertical-menu-btn').on('click', function(event) {
            ...
            // For mobile devices (max-width: 992px)
            if ($(window).width() < 992) {
                $('body').toggleClass('sidebar-enable');
            } else {
                // For desktop devices (min-width: 992px)
                // Toggle between different sidebar sizes
                let currentSize = document.body.getAttribute('data-sidebar-size');
                let newSize;
                ...
                localStorage.setItem('sidebar-size', newSize); // Save to localStorage
            }
        });
    }
    ...
    function initPreloader() {
        $(window).on('load', function() {
            // Hide the new page loader instead of the old preloader
            if (window.pageLoader) {
                window.pageLoader.hide();
            } else {
                // Fallback: hide page-loader element directly
