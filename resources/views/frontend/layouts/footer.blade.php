        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 text-center md:text-left">
            <div>
                ...
                <div class="mb-4">
                    <a href="{{ url('/') }}" class="inline-block">
                        @if($globalSettings && $globalSettings->site_logo)
                            <img src="{{ asset('storage/' . $globalSettings->site_logo) }}" alt="{{ $globalSettings->site_name ?? 'India IVF' }}" class="h-12 md:h-16 w-auto object-contain mx-auto md:mx-0">
                        @else
                            <img src="{{ asset('assets/images/No-1-IVF-Centre-IndiaIVF.webp') }}" alt="India IVF Logo" class="h-12 md:h-16 w-auto object-contain mx-auto md:mx-0">
                        @endif
                    </a>
                </div>
                ...
                <div class="flex justify-center md:justify-start space-x-3 mb-4">
                    ...
                    @if($globalSettings && $globalSettings->youtube_url)
                    <a href="{{ $globalSettings->youtube_url }}" target="_blank" rel="noopener noreferrer" aria-label="YouTube">
                    ...
                    @if($globalSettings && $globalSettings->linkedin_url)
                    <a href="{{ $globalSettings->linkedin_url }}" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn">
            ...
            <div>
                ...
                <div class="space-y-1 text-gray-300 flex flex-col items-center md:items-start">
                    <div class="flex gap-3">
                        ...
                        <p class="font-medium">{{ $globalSettings->site_name ?? 'India IVF' }} Center</p>
                    </div>
                    <p class="text-sm">{{ $globalSettings->contact_address ?? '123 Medical Complex, New Delhi - 110001' }}</p>
                    <div class="flex items-center gap-3">
                        <i class="fas fa-phone text-brand-pink"></i>
                        <span class="hover:text-brand-pink transition-colors cursor-pointer">{{ $globalSettings->contact_phone ?? '+91 12345 67890' }}</span>
                    </div>
                    <div class="flex items-center gap-3">
                        <i class="fas fa-envelope text-brand-pink"></i>
                        <span class="hover:text-brand-pink transition-colors cursor-pointer">{{ $globalSettings->site_email ?? 'info@indiaivf.com' }}</span>
                    </div>
                    @if($globalSettings && $globalSettings->whatsapp_number)
                    <div class="flex items-center gap-3">
                        <i class="fab fa-whatsapp text-green-500"></i>
                        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $globalSettings->whatsapp_number) }}" target="_blank" rel="noopener noreferrer">
                            <span class="hover:text-brand-pink transition-colors cursor-pointer">{{ $globalSettings->whatsapp_number }}</span>
                        </a>
                    </div>
                    @else
                    <div class="flex items-center gap-3">
        ...
        <div class="border-t border-gray-600 mt-6 pt-2">
            <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                <p class="text-gray-300 text-xs text-center md:text-left">
                    @ {{ date('Y') }} {{ $globalSettings->site_name ?? 'India IVF' }}. All rights reserved.
